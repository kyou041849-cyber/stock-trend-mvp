...
- 設定画面でlocalStorage使用量の目安を確認した
- 画面上部にlocalStorage警告が出ていない
- 破損退避キー `stock-trend-mvp:stocks:corrupt:*` が表示された場合は、バックアップJSON復元または退避rawの確認を優先する
