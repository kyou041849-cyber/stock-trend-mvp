...
| G017 | accepted | manager | codex-verifiable | G016 merged to main | none | 裸の4桁ティッカーのJP自動推定 + 市場プレビュー文言の改善 | local validation, PR #14, and GitHub Actions CI succeeded; main merge remains human | [goals/G017_BARE_JP_TICKER_INFERENCE.md](goals/G017_BARE_JP_TICKER_INFERENCE.md) |
...
## G017 Bare JP Ticker Inference Update

Status: accepted, PR #14 CI succeeded; main merge remains human

Outcome target: 市場欄が空のとき、裸の4桁純数字ティッカーをJP / `東証` / `JPY`として推定し、推定不能時の市場プレビュー文言を後から編集できる表現へ改善する。

Implementation:

- `inferMarketRegion` now keeps explicit JP/US market input authoritative before ticker fallbacks.
- Blank-market `.T` tickers and exactly four-digit numeric tickers infer JP.
- `normalizeMarket("", "7203")` returns `東証`.
- `inferCurrency("", "7203")` returns `JPY`.
- Stock form preview now avoids `未推定` and shows an edit-later message when inference is unavailable.

Validation:

- RED: `pnpm run test` failed before the fix because `inferMarketRegion("", "7203")` returned `OTHER`.
- `pnpm run typecheck`: success
- `pnpm run test`: success
- `pnpm run build`: success
- `pnpm run test:e2e -- --reporter=line`: success, 6 passed
- API key / secret scan: no real key found; hits are env var names, docs, existing server-side adapters, and test fake values
...
| ID | Status | Owner | Acceptance | Depends On | Outcome | Evidence |
|---|---|---|---|---|---|---|
| G017 | accepted | manager | codex-verifiable | G016 merged to main | 裸の4桁ティッカーのJP自動推定 + 市場プレビュー文言の改善 | local validation, PR #14, and GitHub Actions CI succeeded; main merge remains human |
